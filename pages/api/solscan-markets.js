...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `https://pro-api.solscan.io/v2.0/token/markets?token[]=${tokenAddress}&page=1&page_size=10`, 
      {
        method: "GET",
        headers: {
    ...
    console.error('Solscan markets API error:', error);
    res.status(500).json({ 
      success: false, 
